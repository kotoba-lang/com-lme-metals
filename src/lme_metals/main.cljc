(ns lme_metals.main
  "Kotodama WASM entrypoint for the Lme_metals clean-room actor (L4) — Clojure port.
  ...
  facts (`lme_metals.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lme_metals")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"lme_metals.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lme_metals-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lme_metals-compat/ready)
